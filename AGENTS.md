...
**语言约定：所有 agent 与贡献者必须以中文进行交流和回复。**
...
Zeus 当前仍处于 beta 阶段，尚无需要维护升级路径的真实用户。实现和重构应优先收敛到当前认可的唯一正确设计，**不要为仓库内曾经出现过的 API、配置、类型或包入口保留兼容层**。

具体规则：

- 旧 API 被新 API 取代时，直接删除旧 API，不保留 alias、转发函数或 deprecated export。
- 数据结构发生变化时，直接迁移仓库内调用方、测试和文档，不同时支持新旧两种格式。
- 包被合并或入口被收敛时，直接删除旧包或旧入口，不设置迁移窗口，不发布兼容转发包。
- 不为尚未发布或仅在仓库内部使用过的行为添加 fallback、warning 或 deprecation 周期。
- 发现仅用于历史兼容的分支、字段、测试或文档时，应在同一改动中删除。

这里所说的“不兼容”只针对 Zeus 自身尚未稳定的历史设计，不等于忽略明确支持的平台、浏览器、模块格式、构建工具或第三方 peer dependency 的真实兼容要求。
