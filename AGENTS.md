...
为保持代码可维护性，遵循以下文件大小限制：

- **TypeScript 文件**：单个文件建议不超过 300 行
- **Rust 文件**：单个文件建议不超过 300 行
- **理由**：
  - 过长的文件难以阅读和维护
  - 便于代码审查（diff 更清晰）
  - 促进模块化和职责分离
  - 便于定位和调试问题
- **超标处理**：当文件接近限制时，应考虑将相关功能拆分为独立模块
...
在 MVP 阶段，代码开发应优先考虑：
- **功能优先**：优先实现核心功能，性能和优化可以后续迭代
- **快速验证**：采用简单直接的方案快速验证想法
- **简化依赖**：避免不必要的抽象和间接层
- **代码可读**：保持代码简洁清晰，便于理解和修改

**无需考虑的限制（MVP 阶段）：**
- 无需考虑浏览器兼容性（假设使用现代浏览器）
- 无需考虑向后兼容
- 无需过度优化性能
- 无需处理边缘情况
- 无需考虑国际化
