...
Zeus 已采用同类思路：组件通过显式 `models` metadata 声明映射，Vue output target 不猜测 `value`/`change`。
...
Zeus manifest 已使用明确的 method schema 记录参数、返回类型与 async 标记；eager element 保留原始同步/异步语义，lazy loader 因加载边界统一返回 Promise。
...
Zeus 采用更直接的 `defineElement` 设计，并支持声明式同步表单值：
...
- `form.value` / `form.state` 会建立响应式 effect，并调用 `internals.setFormValue(value, state)`。
- `form.associated`、`form.disabled`、`form.reset`、`form.stateRestore` 分别接收平台生命周期回调。
- lazy proxy class 也必须带 `static formAssociated = true`，否则浏览器在真实组件加载前不会按表单控件处理。
- lazy proxy 在 constructor 中只调用一次 `attachInternals()`，断开并重连时复用同一实例。
- 真实组件尚未加载时发生的 form lifecycle callback 会排队，并在组件实例创建后按顺序重放。
- `ElementInternals` 不进入 React/Vue wrapper props，它属于 custom element host 能力。

Zeus 对 SSR/不支持 `ElementInternals` 的环境只做 feature detection，不提供行为不完整的 polyfill。
...
- lazy manifest 只传输能力标记与 attribute 名称；真正的 serializer 函数由延迟加载的组件定义执行。
- lazy proxy 会记录值来自 property 还是 attribute，避免加载组件时把未转换的 attribute string 当成最终 property。
- lazy 反射与 proxy attribute 监听共享 reflecting set，避免反射循环。

Zeus 建议：

- object/array 默认 property-only。
- 显式声明 `attr` 后可以使用默认 JSON 转换；公共组件更推荐同时声明 serializer，以固定外部字符串协议。
...
- runtime 能力：`shadow`、`styles`。
- metadata / styling contract：`parts`、`cssVars`，manifest 中表现为 `cssParts`、`cssVars`，供 docs、registry 和工具链使用。
...
Zeus Vue wrapper 当前自己生成 `defineComponent`、`onMounted`、`emit`、prop sync，并消费显式 model metadata 生成 `update:<prop>`。
...
### 4. Vue model metadata

Zeus 不靠 `value` + `value-change` 猜测 v-model，而是使用显式配置：
...
- 监听 model 中声明的 DOM event。
- 保留原始 DOM event 对应的 Vue emit。
- 按 `eventPath` 读取值。
- 额外派发 `update:<prop>`，因此示例配置对应 `v-model:value`。

### 5. Methods / Expose 独立建模

Zeus `ctx.expose(...)` 已进入 manifest：
...
analyzer 会提取内联 expose method 的参数、rest 参数、返回类型与 async 标记，WC/React element d.ts 使用这些签名。Zeus 不强制 eager method 为 async；但 lazy proxy 在组件 chunk 加载完成前无法同步调用，因此 lazy loader d.ts 会把 method 返回值统一包装为 Promise。
...
- lazy custom element：proxy class 带 `formAssociated`，constructor attach internals，真实组件加载后复用并通过 `ctx.internals` 协作。
...
- form callbacks -> `form.associated/disabled/reset/stateRestore`
...
Zeus 应继续沿用“结构化 metadata 驱动多产物”的方式，而不是照搬 Stencil 的内部 flag。当前 `defineElement` / component analyzer 已开始产出统一 manifest，WC、React、Vue、d.ts 和 docs 应继续只消费该结构，避免各输出端重新推断组件协议。

本轮 Zeus 已落地的最小功能闭环：

- `defineElement({ formAssociated: true })` 设置 custom element class 静态表单关联标记。
- setup context 新增 `ctx.internals?: ElementInternals`。
- `form.value/state` 响应式同步 form value，四个平台 form lifecycle callback 有显式配置入口。
- lazy manifest/runtime 传递并应用 `formAssociated`，internals 在重连时保持稳定。
- prop options 新增 `serialize` / `deserialize`。
- eager/lazy 都由真实组件定义执行 serializer，lazy proxy 负责记录来源与抑制反射循环。
- component analyzer 在 manifest 中记录 `meta.formAssociated` 与 prop 的 `serialize` / `deserialize` 标记。
- manifest 新增 models，Vue wrapper 生成 `update:<prop>`。
- expose methods 的参数、返回类型与 async 信息进入 manifest 和生成类型。
