...
| `@zeus-js/preset-component-library` | `packages/web-c/preset-component-library` | 组件库预设，聚合常用输出插件                          |
...
- preset 只做组合，不应隐藏各 output plugin 的核心语义。
...
- 通过 `@zeus-js/preset-component-library` 间接使用。
...
| Component Host        | 组件库作者           | `@zeus-js/bundler-plugin`、`@zeus-js/preset-component-library` |      4 | 组件库构建宿主                |
| Component Outputs     | 组件库作者           | output 插件或 preset                                           |      5 | 多目标产物输出                |
...
- 为集合增加 filter 约定，例如：
  ...
  - Component Host：`@zeus-js/bundler-plugin`、`@zeus-js/component-analyzer`、`@zeus-js/component-dts`、`@zeus-js/preset-component-library`
